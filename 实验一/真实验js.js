...
function initStage(stageKey) {
  ...
  const targetPosts = Array.isArray(window.experimentStimuli)
    ? window.experimentStimuli.filter(
        (post) => post.materialType === stageConfig.materialType,
      )
    : [];

  const stageNeutrals = Array.isArray(window.neutralComments?.high)
    ? {
        high: [...window.neutralComments.high],
        low: [...window.neutralComments.low],
        normal: [...window.neutralComments.normal],
      }
    : { high: [], low: [], normal: [] };

  return {
    config: stageConfig,
    remainingPosts: [...targetPosts],
    currentPost: null,
...
const contrastProduce = {
  timeline: [contrastTrail, distance_measure_1, moral_measure_1, moral_measure_2, emotion_measure, Operational_inspection],
  repetitions: experimentStages.contrastTrail.trialCount,
  randomize_order: true,
};



var taskGroups = [firstProduce, secondProduce, thirdProduce, forthProduce, contrastProduce];
